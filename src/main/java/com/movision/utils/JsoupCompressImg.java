        ...
        try {
            ...
            for (int i = 0; i < titleElms.size(); i++) {
                ...
                if (sum == 0 && s > 800 * 1024) {
                    ...
                    int w = (int) (image.getWidth()*0.50);//对原图尺寸的宽高比压缩50%（待IOS原图传输问题解决后改为20%）
...
//                            newimgurl = PropertiesLoader.getValue("formal.img.domain") + "/" + compressurl;//拿实际url第三个斜杠后面的内容和formal.img.domain进行拼接，如："http://pic.mofo.shop" + "/upload/postCompressImg/img/yDi0T2nY1496812117357.png"
     ...
     * @param mark    IOS标志位：1 IOS发帖 0 非IOS发帖 默认值0
     ...
     * @return
     */
    public Map<String, Object> newCompressImg(HttpServletRequest request, String content, int mark) {
        ...
        try {
            ...
            for (int i = 0; i < moduleArray.size(); i++) {
                ...
                if (type == 1) {
                    ...
                    if (sum == 0 && s > 800 * 1024 && mark == 0) {
                        //1.如果没压缩过 && 2.图片大小超过800kb && 3.非IOS端发帖——就进行压缩，压缩过的或大小<=800kb不处理（防止修改帖子时对压缩过的图片进行重复压缩，同时也保证了低质量图片的品质）
                    ...
                    } else if ((sum == 0 && s <= 800 * 1024) || mark == 1) {
                        //(图片未压缩过且图片大小<=800kb) 或 IOS端发帖
