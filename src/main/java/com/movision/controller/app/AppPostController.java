...
public class AppPostController {
    ...
    public Response releaseModularPost(HttpServletRequest request,
                                       ...
                                       @ApiParam(value = "发帖类型0长图文，1纯图文，2视频(2.2.1以上的版本必填)") @RequestParam(required = false, defaultValue = "0") String category) {

        Response response = new Response();

        Map count = facadePost.postUnderZk(request, userid, circleid, title, subtitle, postcontent, isactive, coverimg, proids, labellist, activeid, mark, category);
...
//    @ApiOperation(value = "新版发帖--长图帖", notes = "新版发帖--长图帖", response = Response.class)
//    @RequestMapping(value = "releasePicturePost", method = RequestMethod.POST)
//    public Response releasePicturePost(HttpServletRequest request,
//                                     @ApiParam(value = "用户id") @RequestParam String userid,
//                                     @ApiParam(value = "所属圈子id") @RequestParam String circleid,
//                                     @ApiParam(value = "所属活动id") @RequestParam(required = false) String activeid,
//                                     @ApiParam(value = "帖子主标题(限18个字以内)") @RequestParam String title,
//                                     @ApiParam(value = "帖子副标题（限制500字符或汉字）") @RequestParam String subtitle,
//                                     @ApiParam(value = "帖子内容") @RequestParam String postcontent,
//                                     @ApiParam(value = "是否为活动：0 帖子 1 活动（APP端固定传0）") @RequestParam String isactive,
//                                     @ApiParam(value = "标签实体集合，json字符串形式") @RequestParam(required = false) String labellist,
//                                     @ApiParam(value = "分享的产品id(多个商品用英文逗号,隔开)") @RequestParam(required = false) String proids){
//        Response response = new Response();
//
//        Map count = facadePost.releasePicturePost(request, userid, circleid, activeid, title, subtitle, postcontent, isactive, labellist, proids);
//
//        if (count.get("flag").equals(-2)) {
//            response.setCode(300);
//            response.setMessage("系统异常，APP发帖失败");
