...
/**
 * 运营中心类目管理 Created by cxx on 2016/3/4 0004.
 */
@RestController
public class SystemController {
    ...
    public Response searchBrandByPager(@RequestParam(required = false) String status, @RequestParam(required = false) String CNName, @RequestParam(required = false) String pageNo,
            @RequestParam(required = false) String pageSize) throws UnsupportedEncodingException {
    ...
    public Response updateBrand(@ModelAttribute CheckBrand brand, @RequestParam String json) {
        ...
        Type type = new TypeToken<ArrayList<CheckSysBrand>>() {
        }.getType();
        ...
        if ("1".equals(brand.getStatus().toString()) || "3".equals(brand.getStatus().toString())) {
            ...
            Type type1 = new TypeToken<ArrayList<SysBrand>>() {
            }.getType();
