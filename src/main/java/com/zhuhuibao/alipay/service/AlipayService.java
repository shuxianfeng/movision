...
public class AlipayService {
        ...
        try {
            ...
            if (verify_result) {// 验证成功
                ...
                if (params.get("trade_status").equals("TRADE_FINISHED") || params.get("trade_status").equals("TRADE_SUCCESS")) {
                    ...
                    // 业务逻辑处理
                    Map<String, String> resultMap = tradeSuccessDeal(params, PayConstants.NotifyType.ASYNC.toString(), tradeType);
                    log.info("***异步回调：支付平台回调发起方支付方结果：" + resultMap);
                    if (resultMap != null && String.valueOf(PayConstants.HTTP_SUCCESS_CODE).equals(resultMap.get("statusCode"))) {
    ...
    public String alipay(Map<String, String> msgParam, String method) {
        ...
        sParaTemp.put("payment_type", aliPayConfig.getPaymentType()); // 支付类型
                                                                      // ，无需修改
        ...
        sParaTemp.put("seller_id", msgParam.get("partner")); // partner =
                                                             // seller_id

        // 业务参数
        sParaTemp.put("goods_type", msgParam.get("alipay_goods_type")); // 商品类型(0:虚拟类商品,1:实物类商品
                                                                        // 默认为1)
    ...
    public String doRefund(Map<String, String> msgParam, String method) {
        ...
        sParaTemp.put("detail_data", detailData); // 退款详细数据
                                                  // 必填(支付宝交易号^退款金额^备注)多笔请用#隔开
        ...
        if (count != Integer.valueOf(batchNum)) {
            ...
            throw new BusinessException(MsgCodeConstant.ALIPAY_PARAM_ERROR, "退款笔数 " + batchNum + " 与退款详情中的记录数 " + count + " 不相等");
        }
    ...
    private void doCallBackBusiness(String tradeType, ModelAndView modelAndView, Map<String, String> params, boolean verify_result) throws ParseException {
        if (verify_result) { // 验证成功
            if (params.get("trade_status").equals("TRADE_FINISHED") || params.get("trade_status").equals("TRADE_SUCCESS")) {
                log.debug("***同步回调：开始处理业务：");
                // 返回成功时业务逻辑处理
                Map<String, String> resultMap = tradeSuccessDeal(params, PayConstants.NotifyType.SYNC.toString(), tradeType);

                log.debug("***同步回调：处理业务结果：" + resultMap);
                if (resultMap != null && String.valueOf(PayConstants.HTTP_SUCCESS_CODE).equals(resultMap.get("statusCode"))) {
