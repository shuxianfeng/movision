...
public class AlipayNotify {
    ...
    private static boolean getSignVeryfy(Map<String, String> Params, String sign) {
        ...
        if (AlipayPropertiesLoader.getPropertyValue("sign_type").equals("RSA")) {
            isSign = RSA.verify(preSignStr, sign, AlipayPropertiesLoader.getPropertyValue("alipay_public_key"), AlipayPropertiesLoader.getPropertyValue("input_charset"));
        }
        return isSign;
    }
     ...
     * @return
     */
    public static boolean h5verify(Map<String, String> params) throws AlipayApiException {
        String responseTxt = "false";
        if (params.get("notify_id") != null) {
            String notify_id = params.get("notify_id");
            responseTxt = verifyResponse(notify_id);
        ...
        String preSignStr = AlipayCore.createLinkString(sParaNew);
        // 获得签名验证结果
        boolean isSign = AlipaySignature.rsaCheckContent(preSignStr, sign, AlipayPropertiesLoader.getPropertyValue("h5_alipay_zfb_public_key"),
                AlipayPropertiesLoader.getPropertyValue("input_charset"));
        boolean ifSuc = isSign && responseTxt.equals("true");
        log.debug("responseTxt=" + responseTxt + "\n isSign=" + isSign + "\n 返回回来的参数：" + AlipayCore.createLinkString(params) + "\n preSignStr：" + preSignStr + "ifSuc:" + ifSuc);
        return isSign && responseTxt.equals("true");
    }
}
