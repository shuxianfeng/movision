...
public class MobileWxPayService {
    ...
    @Autowired
    private OrderGoodsService orderGoodSV;
    ...
    private static final String WEI_XIN_NOTIFY_URL = WxpayPropertiesLoader.getPropertyValue("wei_xin_notify_url");

    private static final String WEI_XIN_QUERY_ORDER_URL = WxpayPropertiesLoader.getPropertyValue("WxpayPropertiesLoader");
    ...
    public Map getWxpayResult(String orderid) throws JDOMException, IOException {
        ...
        Map orderMap = HttpClientUtils.doPostForXml(WEI_XIN_QUERY_ORDER_URL, signParams, "UTF-8");
     ...
     * @throws IOException
     */
    private Map queryOrderResult(Object resultObj) throws JDOMException, IOException {
        Map map = new HashMap<>();
        if (null != resultObj) {
            ...
            if (return_code.equals("SUCCESS")) {
                ...
                if (result_code.equals("SUCCESS")) {
                     ...
                     * //交易状态
                     * if(trade_state.equals(PayConstants.WX_TradeStatus.SUCCESS
                     * .toString())){ }else if(){ }else if(){ }else if(){ }
                     */
    ...
    public ModelAndView handleWxPayNotify(HttpServletRequest request) throws ParseException, JDOMException, IOException {
         ...
         * DistributedLock(LOCK_NAME); lock.lock(); //支付业务处理
         * payHandler(tradeType, modelAndView, requestMap); } catch (Exception
         * e) { log.error("执行异常>>>", e); throw new
         * BusinessException(MsgCodeConstant.SYSTEM_ERROR, e.getMessage()); }
         * finally { if (lock != null) { lock.unlock(); log.info("解锁"); } }
         */
         ...
         */
        /*
         * MutexElement mutex = new MutexElement(); try{
         * mutex.setBusinessNo((String)requestMap.get("out_trade_no"));
         * mutex.setBusinessDesc((String)requestMap.get("transaction_id"));
         * mutex.setTime(1800); mutex.setType("微信支付回调通知"); boolean result =
         * BusinessLockUtil.lock(mutex, 0); //加锁成功 if (result) { //支付业务处理
         * payHandler(tradeType, modelAndView, requestMap); } } catch(Exception
         * e){ log.error("执行异常>>>", e); throw new
         * BusinessException(MsgCodeConstant.SYSTEM_ERROR, e.getMessage()); }
         * finally { //解锁 BusinessLockUtil.unlock(mutex); log.info("解锁"); }
         */
     ...
     * @throws IOException
     */
    private Map getRequestMap(Object returnObj) throws JDOMException, IOException {
    ...
    /**
     * 获取微信支付结果通用通知接口参数 用BufferedReader解析请求，获取map形式的xml
     * 
     * @param request
     * @return
     * @throws JDOMException
     ...
     */
    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    private void payHandler(String tradeType, ModelAndView modelAndView, Map<String, String> requestMap) {
        // 获取返回状态码【通信标识】
     ...
     */
    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    private boolean wxpayNofifySuccessHandle(String tradeType, ModelAndView modelAndView, Map<String, String> requestMap) {
        // 返回成功时业务逻辑处理
        ...
        if (!order.getStatus().equals(PayConstants.OrderStatus.YZF.toString())) {
            if (tradeType.equals(PayConstants.TradeType.PAY.toString())) {
                ...
                OrderFlow orderFlow = orderFlowService.findByOrderNoAndTradeMode((String) requestMap.get("out_trade_no"), PayConstants.PayMode.WXPAY.toString());
    ...
    /**
     * 订单存在，先判断是否存在该订单的流水： 若流水存在，则修改流水的状态=已支付 若流失不存在，则新增已支付的流水
     * 
     * @param requestMap
     * @param orderFlow
     */
    private void handleOrderFlow(Map<String, String> requestMap, OrderFlow orderFlow) {
    ...
    public String getOpenId(String code) {
        ...
        Map<String, String> queryOpenidResult = HttpClientUtils.doGet(GET_OPENID_URL, map, "UTF-8");
     ...
     *            "openid":"o3eXzv14h_YilIZB3JDomt0Zutao","scope":"snsapi_userinfo"},
     *            status=200}
     * @return
     */
    private String parseOpenid(String openid, Map<String, String> queryOpenidResult) {
     ...
     * @throws JDOMException
     */
    public Map<String, String> handleOrder(String openid, String orderid, HttpServletRequest request) throws JDOMException, IOException {
        // 生成随机数
        String nonce_str = SignUtil.generateString(32);
        ...
        prepareParameters(openid, orderid, request, WEI_XIN_NOTIFY_URL, nonce_str, signParams);
        ...
        SortedMap<String, String> jsAPIsignParam = prepareJSAPIParams(nonce_str, doOrderResultMap, orderid);
     ...
     * @throws JDOMException
     */
    private SortedMap<String, String> prepareJSAPIParams(String nonce_str, Map<String, String> doOrderResultMap, String orderid) throws JDOMException, IOException {
        /**
         * 解析结果:返回正确信息
        ...
        if (null != resultObj) {
            ...
            if ("SUCCESS".equals(return_code)) {
                ...
                if (result_code.equals("SUCCESS")) {
                    // 返回成功的处理
                    signAgain = returnSuccessStep(nonce_str, orderid, jsAPIsignParam, map);
                } else {
                    // 返回失败的处理：直接抛出errorcode和errormsg给前台
                    exceptionHandle(map);
                }
     ...
     * @return
     */
    private String returnSuccessStep(String nonce_str, String orderid, SortedMap<String, String> jsAPIsignParam, Map map) {
        String prepay_id;
        String signAgain;
        // 获取到prepay_id
        ...
        OrderFlow orderFlow = prepareOrderFlowParams(orderid, prepay_id, order);

        // 第二次生成签名（该签名需要返回给前端）
        signAgain = genSignAgain(nonce_str, jsAPIsignParam, prepay_id);
     ...
     * @return
     */
    private OrderFlow prepareOrderFlowParams(String orderid, String prepay_id, Order order) {
     ...
     * @return
     */
    private String genSignAgain(String nonce_str, SortedMap<String, String> jsAPIsignParam, String prepay_id) {
     ...
     * @param signParams
     */
    private void prepareParameters(String openid, String orderid, HttpServletRequest request, String wei_xin_notify_url, String nonce_str, SortedMap<String, String> signParams) {
