...
public class MemberRegService {
        ...
        try {
            if (member != null) {
                ...
                if (member.getMobile() != null) {
                    ...
                    member.setStatus(Integer.parseInt(MemberConstant.MemberStatus.WJH.toString()));
                }
            ...
            if (currentTime.before(sendSMStime)) {
                if (info.getCheckCode().equalsIgnoreCase(member.getMobileCheckCode())) {
                    ...
                    if (isExist == 0) {
                        ...
                        // this.getLoginMemberByAccount(member.getMobile());
                        // if(loginMember != null){
                        // Subject currentUser = SecurityUtils.getSubject();
                        // UsernamePasswordToken token = new
                        // UsernamePasswordToken(loginMember.getAccount(),loginMember.getPassword().toCharArray(),true);
    ...
    public LoginMember getLoginMemberByAccount(String memberAccount) {
        ...
        try {
            ...
            } else if (null != loginMember && VipConstant.VipLevel.PERSON_FREE.value == loginMember.getVipLevel() && !MemberConstant.MemberIdentify.GR.toString().equals(loginMember.getIdentify())) {
