...
/**
 * 专家业务处理类 Created by cxx on 2016/5/17 0017.
 */
@Service
@Transactional
public class ExpertService {
	 ...
	 * @return
	 */
	public List<Map<String, String>> findAllExpertSupportListOms(Paging<Map<String, String>> pager,
	...
	public void checkMobileCode(String code, String mobile, String type) {
		if (code != null) {
			...
			if (info != null) {
				Date sendSMStime = DateUtils.date2Sub(DateUtils.str2Date(info.getCreateTime(), "yyyy-MM-dd HH:mm:ss"),
						12, 10);
				// 判断验证码是否过期
				if (currentTime.before(sendSMStime)) {
					...
					if (null != session) {
						...
						if (!code.equals(verifyCode)) {
							throw new BusinessException(MsgCodeConstant.member_mcode_mobile_validate_error,
									MsgPropertiesUtils.getValue(String
					...
					throw new BusinessException(MsgCodeConstant.member_mcode_sms_timeout,
							MsgPropertiesUtils.getValue(String.valueOf(MsgCodeConstant.member_mcode_sms_timeout)));
				}
			} else {
				throw new BusinessException(MsgCodeConstant.member_mcode_mobile_validate_error,
